...
def main():
    dashd = imgcashDaemon.from_imgcash_conf(config.imgcash_conf)

    # check dashd connectivity
    if not is_dashd_port_open(dashd):
        print("Cannot connect to imgcashd. Please ensure imgcash is running and the JSONRPC port is open to Sentinel.")
        return

    # check dashd sync
    if not dashd.is_synced():
        print("imgcashd not synced with network! Awaiting full sync before running Sentinel.")
        return
