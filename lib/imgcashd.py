...
class imgcashDaemon(DashDaemon):

    @classmethod
    def from_imgcash_conf(self, imgcash_dot_conf):
        from imgc_config import imgcashConfig
        config_text = imgcashConfig.slurp_config_file(imgcash_dot_conf)
        creds = imgcashConfig.get_rpc_creds(config_text, config.network)
